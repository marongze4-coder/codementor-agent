...
def configure_console() -> None:
    """Keep Chinese output readable under Windows and redirected terminals."""
...
def main():
    configure_console()
    parser = argparse.ArgumentParser(description="Initialize the EduAgent Milvus collection.")
    parser.add_argument(
        "--recreate",
        action="store_true",
        help="Drop and recreate the collection. Existing vector data will be lost.",
    )
    ...
    print("当前集合：", client.list_collections())
    if args.recreate:
        print("集合已重建，原有数据已清空，请重新运行 build_knowledge_base.py 导入。")
